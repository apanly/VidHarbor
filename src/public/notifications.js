...
function showError(error) { errorRegion.textContent = error instanceof Error ? `${t('common.failed')}: ${error.message}` : formatApiError(error); errorRegion.hidden = false; }
...
async function load() {
  ...
  for (const notification of notifications.items) {
    ...
    const videoCell = document.createElement('td'); videoCell.className = 'notification-video-cell'; videoCell.dataset.label = t('field.video');
    ...
    const channelCell = document.createElement('td'); channelCell.dataset.label = t('field.channel');
    ...
    const publishedCell = document.createElement('td'); publishedCell.dataset.label = t('field.publishedDate'); publishedCell.textContent = notification.video.publishedDate;
    const createdCell = document.createElement('td'); createdCell.dataset.label = t('field.createdAt'); createdCell.textContent = formatChinaTimestamp(notification.createdAt);
    const stateCell = document.createElement('td'); stateCell.dataset.label = t('field.status');
    const readState = document.createElement('span'); readState.className = notification.readAt === null ? 'badge text-bg-warning' : 'badge text-bg-light border'; readState.textContent = notification.readAt === null ? t('notifications.unread') : t('notifications.readAt', { time: formatChinaTimestamp(notification.readAt) }); stateCell.append(readState);
    const actionCell = document.createElement('td'); actionCell.dataset.label = t('field.actions');
    const readButton = document.createElement('button'); readButton.className = 'btn btn-sm btn-outline-primary'; readButton.type = 'button'; readButton.textContent = t('notifications.markRead'); readButton.disabled = notification.readAt !== null; readButton.addEventListener('click', async () => { try { await request(`/api/notifications/${notification.id}/read`, 'POST', {}); location.reload(); } catch (error) { showError(error); } }); actionCell.append(readButton);
