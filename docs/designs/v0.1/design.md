...
- `001-initial.sql` 建立 v0.1 初始结构；`002-manual-channel-sync.sql` 增加人工首次同步状态；`003-generic-direct-downloads.sql` 允许直接下载记录保存 yt-dlp extractor 平台；`004-download-artifacts.sql` 增加时长、缩略图路径和新旧存储布局标记。
...
6. 只有 `succeeded` 且未暂停的频道进入每分钟调度器；后续检查通过 `--dateafter` 固定查询最近 1 个月，边界内没有视频允许成功；新发现的视频生成站内提醒，不自动下载。
...
- 单项直接下载只接受 HTTPS URL，由固定版本 yt-dlp 判断是否支持，并在创建记录前探测恰好一个资源；探测和实际下载都传递 `--no-playlist`。当前契约覆盖 YouTube、Bilibili、Vimeo 和 X 单项地址；Bilibili 通过 `?p=<序号>` 选择分 P，未指定时选择第一 P，X 一帖多视频必须通过 `/video/<序号>` 选择具体媒体。抖音因缺少 cookies 来源不在已验证范围内。
...
- 下载页面按标题在服务端搜索，并固定提供“下载中”和“已完成”两个视图。`completed` 进入已完成，其余状态进入下载中并通过原状态徽标区分；无任务、当前视图为空和搜索无结果分别显示明确空状态。
...
- 下载查询额外接受 `tab=active|completed` 和可选标题 `q`；频道视频查询接受可选标题 `q`。筛选在 SQL 的 `COUNT` 与分页查询中使用同一条件，先筛选再执行 `LIMIT/OFFSET`。
