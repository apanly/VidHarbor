...
- `001-initial.sql` 建立 v0.1 初始结构；`002-manual-channel-sync.sql` 增加人工首次同步状态并保留已有频道数据。
...
- 单视频下载只接受固定 YouTube 视频 URL，并在创建记录前探测元数据。
- 下载先写入 `<downloadRoot>/.vidharbor-tmp/<downloadId>/`，验证唯一、非空、真实普通文件后，以不覆盖目标的原子硬链接归档。
- 用户可以取消活动任务、手工重试失败/取消/中断任务、删除终态记录，以及预览或下载已完成的主文件。
- 下载列表通过 SSE 每秒发送当前快照。活动 SSE 响应由 `RuntimeCoordinator` 登记；定时读取失败会关闭连接并进入运行时失败边界，服务停机也会主动关闭连接。
...
默认测试使用临时 SQLite、临时下载目录和假 yt-dlp/FFmpeg，不访问公网。真实 YouTube 与代理只作为部署前的独立冒烟验证。
